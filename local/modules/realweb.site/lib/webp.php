...
/**
 * Class \Realweb\Api\Module\Site\Model\Webp
 *
 */
class Webp {
    /*
    ...
    public static function convertAllToWebp(&$content) {
        ...
        if (self::checkSupport() && !Site::isMobileIOS()) {
            $arPatterns = array(
            ...
            foreach ($arPatterns as $strPattern) {
                ...
                foreach ($arMatches[1] as $keyMatch => $strPath) {
                    ...
                    if (stripos($strPath, 'instagram') === false) {
    ...
    public function getWebpPath() {
        ...
        if ($this->isPng()) {
            ...
            if ($this->_isPhp56BetaEnabled()) {
                ...
                $strTempSrc = str_replace('.png', '_tmp.jpg', $this->getSourceFile()->getPath());
