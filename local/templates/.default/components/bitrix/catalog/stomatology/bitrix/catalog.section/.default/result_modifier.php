...
$priceLists = CIBlockSection::GetMixedList(['DEPTH_LEVEL'=>'ASC'], ['SECTION_ID'=>$priceSectionId], false, ['NAME', 'IBLOCK_SECTION_ID', 'ID', 'DEPTH_LEVEL', 'PROPERTY_PRICE']);
while($price = $priceLists->GetNext()){
    if($price['TYPE'] == 'E') {
        $priceList['ITEMS'][] = $price;
    } else {
        $priceList['SUBSECTION'][$price['ID']]['NAME'] = $price['NAME'];
        $subsectionItems = CIBlockElement::GetList(['SORT'=>'ASC'], ['SECTION_ID' => $price['ID']], false, false, ['IBLOCK_ID', 'NAME', 'ID', 'PROPERTY_PRICE']);
